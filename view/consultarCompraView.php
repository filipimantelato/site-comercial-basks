...
<body>
    ...
    <div class="corpoCadastroProdutos corpoConsultaProdutos">
        <h1>Consulta de Compras</h1>
        <form action="../controller/consultarCompraController.php" method="post">
            <label for="email">Digite o e-mail do Cliente: </label>
            <input type="text" name="email" size="30" class="email" placeholder="exemplo@gmail.com">
            <label for="data">Data da Compra: </label>
            <input type="date" name="data" class="data">
            <input type="hidden" name="consulta" value="compra">
            <input type="submit" value="Consultar" class="botao"><br><br><br>
        </form>

        <?php   
            require_once '../model/consultarCompraModel.php';
            $consulta = new ConsultarComprasModel();
            $result = $consulta->consultarTodasCompras();
            
            if ($result->num_rows > 0) {
                echo '<table class="table">';
                echo '<thead style="background-color: #FF7F50">
                <tr>
                    <th>Id da Compra</th>
